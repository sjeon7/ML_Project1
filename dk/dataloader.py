...
#        self.metadata_df = self.metadata_df.reindex(np.random.permutation(self.metadata_df.index))
    ...
    def next_batch(self, feature):
        ...
        start_pos = (self.pointer - 1) * self.batch_size
...
#        valid_ids, valid_features = features.compute_mfcc_example(track_ids)
        ...
        valid_features = feat.iloc[:,1:]
        
        valid_df = meta_df[meta_df['track_id'].isin(np.array(valid_ids).flatten())]
#        valid_df = meta_df[meta_df['track_id'].isin(valid_ids)]
#        return valid_features, self.convert_labels(valid_df)
        return np.array(valid_features), self.convert_labels(valid_df)
