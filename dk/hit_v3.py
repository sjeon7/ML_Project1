...
with slim.arg_scope([slim.conv2d, slim.fully_connected],
    ...
    net = slim.max_pool2d(net, [2, 4], stride=[2, 2], scope='pool1')
#    net = tf.nn.dropout(net, 0.5)
    net = slim.conv2d(net, 128, [3, 3], padding='same', scope='conv2')
    net = slim.max_pool2d(net, [2, 4], stride=[2, 2], scope='pool2')
#    net = tf.nn.dropout(net, 0.5)
    net = slim.conv2d(net, 128, [3, 3], padding='same', scope='conv3')
    net = slim.max_pool2d(net, [2, 4], stride=[2, 2], scope='pool3')
#    net = tf.nn.dropout(net, 0.5)
    net = slim.conv2d(net, 128, [3, 3], padding='same', scope='conv4')
    net = slim.max_pool2d(net, [2, 4], stride=[2, 2], scope='pool4')
#    net = tf.nn.dropout(net, 0.5)
    net = slim.conv2d(net, 64, [3, 3], padding='same', scope='conv5')
#    net = slim.max_pool2d(net, [2, 4], stride=[2, 2], scope='pool5')    
#    net = tf.nn.dropout(net, 0.5)
...
print(net)
weights = tf.Variable(tf.random_normal([64, 3]), name='weights')
biases = tf.Variable(tf.random_normal([3]), name='biases')

lstm_input = tf.reshape(net, [batch_size, 154, 64])
#lstm_input = tf.reshape(net, [batch_size, 11, 14*64])
lstm_input = tf.unstack(lstm_input, axis=1)
lstm_cell_basic = rnn_cell.BasicLSTMCell(64)
lstm_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell_basic] * 5)
outputs, _ = rnn.static_rnn(lstm_cell, lstm_input, dtype=tf.float32, scope='lstm1')
outputs = tf.stack(outputs)
#outputs = tf.transpose(outputs, [1, 0, 2])
outputs = tf.gather(outputs, int(outputs.get_shape()[0]) - 1)
flat = tf.reshape(outputs, [-1, weights.get_shape().as_list()[0]])
pred = tf.nn.softmax(tf.matmul(flat, weights) + biases)
...
with tf.Session() as sess:
    ...
    if is_train_mode:
        ...
        validation_dataloader = DataLoader(file_path='dataset/track_metadata.csv', batch_size = batch_size, 
...
#            train_dataloader.shuffle_df()
            ...
            validation_dataloader.reset_pointer()
